...
df_old <- 
  ...
  mutate(
    print_mass = str_extract(print_mass_grams, "\\d+"), # extract just mass from print mass
    printer = tolower(printer) #make class lowercase
  ) %>% 
  #separate into date and time
  separate(timestamp, c("date", "time"), sep = " ", remove = FALSE) %>% 
  transform(
...
df_old <-
  ...
  mutate (
    ...
    # printer = str_replace(printer, ".*[^himalia].*|.*[^elera].*|.*[^ganymede].*|.*[^callisto].*|.*[^thebe].*|.*[^themisto].*|.*[^io].*|.*[^metis].*|.*[^sinope].*|.*[^dia].*|.*[^europa].*|.*[^queue].*|.*[^prusa].*|.*[^valetudo].*|.*[^valetudo].*", "other")
  #   printer = str_replace(printer, "^(?!himalia).*$|^(?!lera).*$|^(?!ganymede).*$|^(?!callisto).*$|^(?!thebe).*$|^(?!themisto).*$|^(?!io).*$|^(?!metis).*$|^(?!sinope).*$|^(?!dia).*$|^(?!europa).*$|^(?!queue).*$|^(?!prusa).*$|^(?!valetudo).*$|^(?!amalthea).*$", "other")
  # )
    # class = factor(
    #   class,
    #   levels = c("pie", "dfm", "scope", "is", "desnat", "renewable energy", "eec", "mech proto", "qea", "research", "p&m", "sustdes", 
